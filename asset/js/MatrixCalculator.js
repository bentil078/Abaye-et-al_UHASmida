class MatrixCalculator {
		...
		for (row = 0; row < rank; row++) { 
			if (mat[row][row]) { 
			   for (var col = 0; col < this.AyDimension; col++) { 
				   if (col != row) { 
					 var mult = Math.round(mat[col][row] / mat[row][row]*10000)/10000; 
	...
	invertMatrix() {		
		...
		if (this.AxDimension!=1) {
			for (var i =0; i<this.AxDimension; i++) {
				for (var j=0; j<this.AyDimension; j++) {
					result[i][j]=Math.round(result[i][j]/this.determinantA*10000)/10000;
				}
			}
		}
	...
	subtractMatrix() {
		...
		for (i =0; i<this.AyDimension; i++) {
			for (var j=0; j<this.AxDimension; j++) {
				result[i][j]=Math.round((parseFloat(this.matrixA[i][j])-parseFloat(this.matrixB[i][j]))*10000)/10000;
			}
		}
	...
	multiplyMatrix() {
		...
		for (i=0; i<rowsRes; i++) {
			for (j=0; j<columnsRes; j++) {
				...
				result[i][j] = Math.round(result[i][j]*10000)/10000;
			}
		}
	...
	calculateDeterminant() {
		...
		if (this.AxDimension==3) {
			...
			determinant = Math.round((op1+op2+op3-r1-r2-r3)*10000)/10000;
		}
